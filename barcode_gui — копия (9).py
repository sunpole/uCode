...
# ============================================
# ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (только для PNG)
# ============================================
# Для PNG мы используем стандартный ImageWriter (он не поддерживает длинные полосы,
# но для быстрой визуализации сойдёт). Основной фокус – на PDF.
...
class BarcodeApp:
    ...
    def create_widgets(self):
        ...
        ttk.Entry(cmyk_frame, textvariable=self.cmyk_c, width=4).pack(side=tk.LEFT, padx=2)
        ttk.Label(cmyk_frame, text="M:").pack(side=tk.LEFT)
        ttk.Entry(cmyk_frame, textvariable=self.cmyk_m, width=4).pack(side=tk.LEFT, padx=2)
        ttk.Label(cmyk_frame, text="Y:").pack(side=tk.LEFT)
        ttk.Entry(cmyk_frame, textvariable=self.cmyk_y, width=4).pack(side=tk.LEFT, padx=2)
        ttk.Label(cmyk_frame, text="K:").pack(side=tk.LEFT)
        ttk.Entry(cmyk_frame, textvariable=self.cmyk_k, width=4).pack(side=tk.LEFT, padx=2)
    ...
    def generate_one(self, code, output_dir, fmt):
        ...
        if fmt == "png":
            # PNG через ImageWriter (простой, без длинных полос)
            writer = ImageWriter()
            dpi = 300
            ...
            # PDF: используем SVGWriter, затем модифицируем
            writer = SVGWriter()
            writer.set_options({
                'module_width': self.module_width.get(),
                'module_height': self.short_height.get(),  # пока короткая высота
                'font_size': self.font_size.get(),
                'text_distance': self.text_distance.get(),
                'quiet_zone': 0,  # мы добавим свои отступы позже
                'background': self.bg_color.get(),
                'foreground': self.fg_color.get(),
            })
            ean = EAN13(code, writer=writer)
            ...
            # Модифицируем SVG: увеличиваем высоту для старт, центр, стоп
            try:
                ...
                # Фильтруем только штрихи (цвет fg_color)
                fg = self.fg_color.get()
                bar_rects = [r for r in rects if r.get('fill') == fg]
                if bar_rects:
                    module_w = self.module_width.get()
                    # Перебираем все rect-штрихи
                    for rect in bar_rects:
                        x = float(rect.get('x', 0))
                        # Определяем, попадает ли в длинные зоны (старт, центр, стоп)
                        # Старт: 0..3*module_w, центр: 45..50, стоп: 92..95
                        if (x >= 0 and x < 3 * module_w) or \
                           (x >= 45 * module_w and x < 50 * module_w) or \
                           (x >= 92 * module_w and x < 95 * module_w):
                            # Увеличиваем высоту до long_height
                            rect.set('height', str(self.long_height.get()))
                    # Сохраняем изменения
                    tree.write(svg_path)
            except Exception as e:
                # Если не удалось модифицировать, всё равно продолжаем
                pass
