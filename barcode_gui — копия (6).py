...
def generate_ean13_svg(code12: str, module_width: float, short_height: float, long_height: float,
    ...
    # Базовая позиция текста по Y: низ коротких полос + отступ
    text_y = short_height + text_distance

    # Первая цифра (слева, выравнивание по правому краю)
    # Её центр – на расстоянии примерно половины левого отступа
    first_x = left_margin - (left_margin / 2)  # примерно посередине левого поля
    t1 = font_props.copy()
...
class BarcodeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Генератор штрихкодов EAN-13 (ГОСТ / GS1)")
        self.root.geometry("1000x900")
        self.root.resizable(True, True)

        # Переменные настроек (по ГОСТу)
        self.output_format = tk.StringVar(value="pdf")
        self.module_width = tk.DoubleVar(value=0.330)      # номинал
        self.short_height = tk.DoubleVar(value=22.85)      # номинал
        self.long_height = tk.DoubleVar(value=24.50)       # номинал (длиннее на 1.65 мм)
        self.left_margin = tk.DoubleVar(value=3.63)        # 11 * 0.330 = 3.63
        self.right_margin = tk.DoubleVar(value=2.31)       # 7 * 0.330 = 2.31
        self.font_size = tk.DoubleVar(value=2.75)          # номинал
        self.text_distance = tk.DoubleVar(value=0.50)      # отступ текста от низа коротких полос (рекомендуется 0.5 мм)
        self.bg_color = tk.StringVar(value="#ffffff")      # белый
        self.fg_color = tk.StringVar(value="#000000")      # чёрный (CMYK 0,0,0,100)

        # Дополнительные переменные для CMYK (показываем для информации)
        ...
        # Привязываем изменение module_width к автоматическому пересчёту отступов (по желанию)
        # Но мы не будем автоматически менять отступы, чтобы пользователь мог их регулировать отдельно.
    ...
    def create_widgets(self):
        ...
        right_frame = ttk.LabelFrame(main_frame, text="Настройки генерации (ГОСТ по умолчанию)", padding="5")
        ...
        # Отображаем выбранный цвет (квадратик)
        ...
        # Информация о CMYK
        cmyk_frame = ttk.LabelFrame(right_frame, text="CMYK (для информации, только чёрный по умолчанию)")
    ...
    def apply_cmyk(self):
        ...
        # Простое преобразование CMYK -> RGB (без профиля)
        r = int(255 * (1 - c) * (1 - k))
        g = int(255 * (1 - m) * (1 - k))
        b = int(255 * (1 - y) * (1 - k))
    ...
    def choose_font(self):
        ...
        if font_path:
            ...
            # Для SVG мы будем использовать системное имя, но для PNG можно использовать этот шрифт
    ...
    def generate_one(self, code, output_dir, fmt):
        ...
        if fmt == "png":
            ...
            writer = ImageWriter()
            # Приблизительно пересчитываем параметры в пиксели (для примера)
            dpi = 300
