...
class BarcodeApp:
    ...
    def preview_first(self):
        ...
        base_name = os.path.join(temp_dir, "preview_barcode")
        try:
            if fmt == "pdf":
                writer = self.build_writer("pdf")
                ean = EAN13(code, writer=writer)
                saved = ean.save(base_name)  # сохранит с расширением .pdf
                # Открываем системным просмотрщиком
                self.open_file(saved)
            else:
                # Для PNG используем ImageWriter с текстом (стандартный шрифт)
                writer = self.build_writer("png")
                ean = EAN13(code, writer=writer)
                saved = ean.save(base_name)  # .png
                self.open_file(saved)
            self.status_label.config(text=f"Предпросмотр создан: {os.path.basename(saved)}", foreground="green")
        except Exception as e:
