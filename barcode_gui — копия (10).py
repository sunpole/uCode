...
import cairosvg  # новая зависимость для конвертации SVG → PDF
...
def show_splash():
    """Показывает заставку на 3 секунды, затем закрывает её."""
    splash = tk.Tk()
    ...
    label_ver = tk.Label(splash, text="Версия 1.0.2", font=("Arial", 12), fg="#95a5a6", bg="#2c3e50")
    ...
    # Закрываем заставку через 3 секунды и запускаем основное окно
...
class BarcodeApp:
    ...
    def create_widgets(self):
        ...
        ttk.Label(info_frame, text="uCode Barcode Generator v1.0.2", font=("Arial", 14, "bold")).pack(side=tk.LEFT)
    ...
    def generate_one(self, code, output_dir, fmt):
        ...
        if fmt == "png":
            ...
            # PDF: SVGWriter + модификация высоты
            writer = SVGWriter()
            writer.set_options({
                'module_width': self.module_width.get(),
                'module_height': self.short_height.get(),
            ...
            # Модифицируем SVG: удлиняем старт, центр, стоп
            try:
                ...
                # Если не удалось модифицировать, продолжаем с тем, что есть
                pass

            # Конвертируем SVG в PDF с помощью cairosvg
...
if __name__ == "__main__":
    # Показываем заставку, которая через 3 секунды запустит основное окно
    show_splash()